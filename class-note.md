...
**目标：**
配置和验证 Istio 的安全特性，例如 mTLS 和授权策略。

1. **边界层安全：Ingress TLS 配置** （Lab 6）
   - 配置双向 TLS，结合 SDS（Secret Discovery Service）。

2. **内部服务安全：mTLS 配置** （Lab 7）
...
**目标：**
使用 Istio 的监控工具收集服务调用链与性能指标。

1. **分布式调用链与日志分析** （Lab 9.1, 9.4）
   - 使用 Jaeger 和 Envoy 日志追踪调用链。

2. **错误与请求监控** （Lab 9.2, 9.3）
